    ...
    <div className='characters-manager'>
      ...
      <div className='characters-manager__grid'>
        {disneyCharacters.map((character) => (
          <Character key={character._id} className='characters-manager__item' character={character} handleClick={(newCharacterDetailId) => setCharacterDetailId(newCharacterDetailId)}></Character>
        ))}
      </div>

      <div className='characters-manager__pager'>
        <button disabled={currentPage === 1} className='characters-manager__pager-button' onClick={() => setCurrentPage(currentPage - 1)}>
          Anterior
        </button>
        <span className='characters-manager__current'>{currentPage}</span>
        <button disabled={currentPage === maxPages} className='characters-manager__pager-button' onClick={() => setCurrentPage(currentPage + 1)}>
          Siguiente
        </button>
      </div>
      ...
      {characterDetailInfo ? (
        <CharacterDetail
          ...
          handleClick={(newCharacterDetailId) => {
